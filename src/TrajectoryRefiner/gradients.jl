...
- `traj_cost_fn`: Function `x -> cost` mapping rollout trajectories `(state_dim, seq_len, batch)` to a scalar or per-batch cost.
...
function refinement_loss(network::RefinementModel, transition_fn, traj_cost_fn, mismatch_fn,
    ...
    objective_loss = mismatch_fn(x_res, x_body) + traj_cost
...
function refinement_grads(network::RefinementModel, transition_fn, traj_cost_fn, mismatch_fn,
    ...
    grads = Flux.gradient(network) do m
        l = refinement_loss(m, transition_fn, traj_cost_fn, mismatch_fn, sample, total_steps;
